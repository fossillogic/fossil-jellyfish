    ...
    for (size_t i = 0; i < chain->count; ++i) {
        ...
        switch (block->block_type) {
            case JELLY_BLOCK_BASIC:
                ...
                if (block->imagined) {
                    printf("[AUDIT] Block %zu: BASIC block marked as imagined.\n", i);
                    issues++;
                }
                if (strlen(block->input) == 0 || strlen(block->output) == 0) {
                    printf("[AUDIT] Block %zu: BASIC block missing input or output.\n", i);
                    issues++;
                }
                if (block->confidence < 0.0f || block->confidence > 1.0f) {
                    printf("[AUDIT] Block %zu: BASIC block has invalid confidence %.2f.\n", i, block->confidence);
                    issues++;
                }
                break;
            case JELLY_BLOCK_IMAGINED:
                ...
                if (!block->imagined) {
                    printf("[AUDIT] Block %zu: IMAGINED block not marked as imagined.\n", i);
                    issues++;
                }
                if (strlen(block->imagination_reason) == 0) {
                    printf("[AUDIT] Block %zu: IMAGINED block missing imagination reason.\n", i);
                    issues++;
                }
                break;
            case JELLY_BLOCK_DERIVED:
                ...
                if (block->imagined && block->imagined_from_index == 0) {
                    printf("[AUDIT] Block %zu: DERIVED block imagined but missing source index.\n", i);
                    issues++;
                }
                if (block->imagined && strlen(block->imagination_reason) == 0) {
                    printf("[AUDIT] Block %zu: DERIVED block imagined but missing reason.\n", i);
                    issues++;
                }
                break;
            case JELLY_BLOCK_EXPERIMENTAL:
                ...
                if (block->confidence > 0.9f) {
                    printf("[AUDIT] Block %zu: EXPERIMENTAL block has suspiciously high confidence.\n", i);
                    issues++;
                }
                break;
            case JELLY_BLOCK_VERIFIED:
                ...
                if (!block->immutable) {
                    printf("[AUDIT] Block %zu: VERIFIED block not marked immutable.\n", i);
                    issues++;
                }
                if (!fossil_jellyfish_block_verify_signature(block, block->device_id)) {
                    printf("[AUDIT] Block %zu: VERIFIED block signature verification failed.\n", i);
                    issues++;
                }
                break;
            default:
                printf("[AUDIT] Block %zu: Unknown block type %d.\n", i, block->block_type);
                issues++;
                break;
        }
    }
